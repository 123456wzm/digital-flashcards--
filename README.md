...
- 卡片列表显示「最近更新：XX」，编辑器顶部显示创建时间和更新时间
...
- **时间排序** — 支持按创建时间或更新时间排序，可切换正序（最早优先）/ 倒序（最新优先）
- 所有筛选条件可叠加使用（搜索 AND 标签 AND 日期）

### 数据管理
- **导入导出** — 支持 JSON 格式备份和恢复所有数据（含图片）
- **批量操作** — 多选卡片后全选、批量删除、批量添加标签
...
| 版本 | 说明 |
|------|------|
| v1.6 | 重设计排序功能，支持按创建/更新时间排序，优化筛选栏和编辑器 UI |
| v1.5 | 添加全选功能 |
| v1.4 | 增加日期范围筛选和时间排序功能 |
| v1.3 | 增加按分类标签筛选功能 |
| v1.2 | 增加 WebDAV 同步支持 |
| v1.1 | 增加版本历史功能 |
| v1.0 | 初始版本 |
